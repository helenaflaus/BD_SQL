...
INSERT INTO sala (situacao_sala, nome_sala, tipo_video, capac_sala, tipo_audio)
VALUES ('ATIVA', 'Kubrick', '3D', 150, 'DTS');

INSERT INTO sala (situacao_sala, nome_sala, tipo_video, capac_sala, tipo_audio)
VALUES ('MANUTENCAO', 'Tarantino', '2D', 120, 'Dolby');

INSERT INTO sala (situacao_sala, nome_sala, tipo_video, capac_sala, tipo_audio)
VALUES ('ATIVA', 'Hitchcock', 'IMAX', 250, 'IMAX');

INSERT INTO sala (situacao_sala, nome_sala, tipo_video, capac_sala, tipo_audio)
VALUES ('INATIVA', 'Spielberg', '4D', 180, 'Dolby Atmos');

INSERT INTO sala (situacao_sala, nome_sala, tipo_video, capac_sala, tipo_audio)
VALUES ('ATIVA', 'Scorsese', '2D', 200, 'DTS');

INSERT INTO sala (situacao_sala, nome_sala, tipo_video, capac_sala, tipo_audio)
VALUES ('ATIVA', 'Nolan', '3D', 220, 'Dolby');

INSERT INTO sala (situacao_sala, nome_sala, tipo_video, capac_sala, tipo_audio)
VALUES ('MANUTENCAO', 'Fincher', '2D', 140, 'Dolby Surround');

INSERT INTO sala (situacao_sala, nome_sala, tipo_video, capac_sala, tipo_audio)
VALUES ('INATIVA', 'Cameron', 'IMAX', 300, 'IMAX');

INSERT INTO sala (situacao_sala, nome_sala, tipo_video, capac_sala, tipo_audio)
VALUES ('ATIVA', 'Kubrick', '2D', 160, 'Dolby');

INSERT INTO sala (situacao_sala, nome_sala, tipo_video, capac_sala, tipo_audio)
VALUES ('ATIVA', 'Coen', '3D', 180, 'DTS');
...
UPDATE filme
SET titulo_original = 'Madagascar'
WHERE cod_filme = 1;
SELECT *
FROM filme ;
-- populando filme
...
'EM CARTAZ', 'Drama', 'LIVRE', 'Disney', 'EUA' );
...
INSERT INTO escala_trabalho VALUES ( 14, 4, 'segunda-feira', 'GERENTE') ;
...
FROM sessao ss  RIGHT OUTER JOIN filme f -- (FILME - SESSAO)
    ON ( f.cod_filme = ss.cod_filme)
WHERE ss.cod_filme IS NULL ;
--35 Diferença usando o operador MINUS
...
--38 outra forma usando select aninhado
...
( 5, 'Hebe Camargo','F', '10/01/1949', 
 ...
 --39 Mostrar os funcionarios que ainda não foram escalados
...
--40 Subconsulta ou Subquery ou Select aninhado
...
/* Atividade 06 - Utilizando o comando SELECT do SQL:
1 – Mostre os filmes que nunca tiveram premiação: Título Filme – Gênero – Classificação Etária */
SELECT f.titulo_original, f.genero, f.class_etaria, f.cod_filme AS PKfilme, 
pr.cod_filme AS FKPremiacao
FROM filme f LEFT JOIN premiacao pr
     ON (f.cod_filme = pr.cod_filme ) -- PK filme --> FK Premiação
WHERE pr.cod_filme IS NULL ;

/* 2 – Cadastre uma nova sala. Mostre as salas que nunca tiveram sessões dubladas
para filmes de Ação ou Aventura produzidas pelos EUA:
Nome da Sala – Tipo de Áudio – Capacidade lugares. 
Resolva de três formas diferentes (junção externa, operador diferença e NOT IN) */
INSERT INTO sala VALUES ('Tarantino', 250, 'Dolby Surround', '3D Miramax', 'ATIVA') ;
SELECT * FROM sala ;
SELECT * FROM filme ;
-- junção externa
SELECT s1.nome_sala, s1.tipo_audio, s1.capacidade_sala, sala_acao_eua.nome_sala
FROM sala s1 LEFT JOIN 
   ( SELECT ss.nome_sala FROM sala s JOIN sessao ss 
                        ON (s.nome_sala = ss.nome_sala ) 
	                                JOIN filme f  
	                    ON ( f.cod_filme = ss.cod_filme)
            WHERE ( f.genero ILIKE '%a__o%' OR f.genero ILIKE '%aventura%')
            AND f.nacionalidade = 'EUA'
            AND ss.legendado ) sala_acao_eua
	ON (s1.nome_sala = sala_acao_eua.nome_sala )
WHERE sala_acao_eua.nome_sala IS NULL ;
-- diferença
SELECT s2.nome_sala, s2.tipo_audio, s2.capacidade_sala
FROM sala s2 
WHERE s2.nome_sala IN ( 
                       SELECT s1.nome_sala
                       FROM sala s1
                           EXCEPT 
                                 ( SELECT s.nome_sala FROM sala s JOIN sessao ss 
                                          ON (s.nome_sala = ss.nome_sala ) 
	                                JOIN filme f  
	                                      ON ( f.cod_filme = ss.cod_filme)
                                   WHERE ( f.genero ILIKE '%a__o%' OR f.genero ILIKE '%aventura%')
                                   ...
                                   AND ss.legendado ) ) ;
-- NOT IN 
SELECT s1.nome_sala, s1.tipo_audio, s1.capacidade_sala -- todas as salas
  FROM sala s1
  WHERE s1.nome_sala NOT IN
             ( SELECT ss.nome_sala FROM sala s JOIN sessao ss -- as salas que exibiram EUA
                      ON (s.nome_sala = ss.nome_sala )       -- legendado (não dublado)
	                      JOIN filme f                       -- Ação ou Aventura
	                  ON ( f.cod_filme = ss.cod_filme)
               WHERE ( f.genero ILIKE '%a__o%' OR f.genero ILIKE '%AveNtuRa%')
                     AND f.nacionalidade = 'EUA'
                     AND ss.legendado ) ;

/* 3 – Mostre o filme norte-americano com a menor duração: Título Filme – Duração – Gênero */
SELECT f1.titulo_original, f1.duracao_min, f1.genero
FROM filme f1
WHERE f1.nacionalidade = 'EUA'
AND f1.duracao_min = ( SELECT MIN(f2.duracao_min)
                        FROM filme f2 WHERE f2.nacionalidade = 'EUA') ; 

/* 4 - Mostre o filme e a sessão que tiveram a maior taxa de ocupação:
Título Filme - Número da sessão – Data - Horário – Publico – Capacidade da Sala – Taxa de Ocupação */
SELECT f.titulo_original, ss.num_sessao, ss.dt_sessao, h.hora AS Horario, 
ss.publico, s.capacidade_sala, 
TRUNC((ss.publico::NUMERIC/s.capacidade_sala::NUMERIC)*100,0) As Taxa
FROM filme f JOIN sessao ss ON ( f.cod_filme = ss.cod_filme)
    JOIN sala s ON (s.nome_sala = ss.nome_sala ) 
    JOIN horario h ON (h.cod_horario = ss.cod_horario)
WHERE ss.num_sessao IN
  ( SELECT ss.num_sessao -- recupera o numero da sessao com maior taxa
    FROM sessao ss JOIN sala s ON (s.nome_sala = ss.nome_sala )
    WHERE (ss.publico::NUMERIC/s.capacidade_sala::NUMERIC)*100 = 
       (SELECT MAX((ss.publico::NUMERIC/s.capacidade_sala::NUMERIC)*100) As Maior_taxa -- maior taxa
         FROM sessao ss JOIN sala s ON (s.nome_sala = ss.nome_sala ) ) ) ;

/* 5 – Mostre as sessões deste ano que tiveram publico maior que a média de público (este ano):
Número da sessão – Data - Horário – Titulo Filme - Público – Média de Público. */
SELECT ss.num_sessao, ss.dt_sessao AS Data,  h.hora AS Horario, f.titulo_original, 
ss.publico, ( SELECT AVG(ss.publico) AS Media
               FROM sessao ss
...
WHERE EXTRACT(YEAR FROM ss.dt_sessao) = EXTRACT(YEAR FROM current_date)
AND ss.publico > 
                 ( SELECT AVG(ss.publico) AS Media  -- media de publico ano atual
                   FROM sessao ss
                   WHERE EXTRACT(YEAR FROM ss.dt_sessao) = EXTRACT(YEAR FROM current_date) ) ;

/* 6 – Mostre todos os dados da sessão com maior rentabilidade (renda/publico). */
SELECT ss.*
FROM sessao ss
WHERE ROUND((ss.renda_sessao::NUMERIC/ss.publico::NUMERIC),1) =   -- 92.59
   (SELECT ROUND(MAX(ss.renda_sessao::NUMERIC/ss.publico::NUMERIC),1) As Maior_rentabilidade -- maior rentabilidade
           FROM sessao ss ) ;
SELECT ss.*
FROM sessao ss
WHERE TRUNC((ss.renda_sessao::NUMERIC/ss.publico::NUMERIC),1) =   -- 92.59
   (SELECT TRUNC(MAX(ss.renda_sessao::NUMERIC/ss.publico::NUMERIC),1) As Maior_rentabilidade -- maior rentabilidade
           FROM sessao ss ) ;
		   
/**** Aula 28/maio - Consultas Avançadas (técnicas diferentes)   ***/
-- 41 - Exemplo de OUTER JOIN com consulta aninhada virando tabela temporária
-- Mostrar o nome das salas que nunca exibiram filmes de DRAMA
-- Estratégia : encontrar as salas que exibiram filmes de DRAMA e 
-- fazer a diferença com todas as salas
SELECT sat.nome_sala AS TodasSalasPK, SalasDramaticas.SalasFK
FROM sala sat LEFT JOIN 
                        ( SELECT sa.nome_sala AS SalasFK
                           FROM sessao ss JOIN sala sa ON (ss.nome_sala = sa.nome_sala)
                            JOIN filme f ON (ss.cod_filme = f.cod_filme)
                             WHERE f.genero ILIKE '%drama%') SalasDramaticas
ON ( sat.nome_sala = SalasDramaticas.SalasFK )
WHERE SalasDramaticas.SalasFK  IS NULL ;
-- NOT IN 
SELECT sat.nome_sala AS TodasSalasPK
FROM sala sat 
WHERE sat.nome_sala NOT IN 
                        ( SELECT sa.nome_sala AS SalasFK
                           FROM sessao ss JOIN sala sa ON (ss.nome_sala = sa.nome_sala)
                            JOIN filme f ON (ss.cod_filme = f.cod_filme)
                             WHERE f.genero ILIKE '%drama%') ;
-- EXCEPT 
SELECT sat.nome_sala AS TodasSalasPK -- todas as salas
FROM sala sat 
EXCEPT
        ( SELECT sa.nome_sala AS SalasFK -- salas que exibiram drama
          FROM sessao ss JOIN sala sa ON (ss.nome_sala = sa.nome_sala)
          JOIN filme f ON (ss.cod_filme = f.cod_filme)
          WHERE f.genero ILIKE '%drama%') ;
-- 42 - Uso do CASE -> correspondo ao IF THEN ELSIF ELSE
SELECT * FROM Filme ;
SELECT * FROM sessao ;

SELECT f.titulo_portugues, f.ano_lancamento,
CASE UPPER(f.nacionalidade)
     WHEN 'EUA' THEN 'Norte-Americano'
	 WHEN 'BRASIL' THEN 'Brasileiro'
	 ELSE 'Outras Nacionalidades'
END AS "Nacionalidade Produtor"
FROM filme f ;

-- 43 - CASE para a coluna Legendado da Sessao - Booleano : Legendada(true) ou Dublada (false)
SELECT ss.num_sessao AS Sessao, ss.dt_sessao AS Data, f.titulo_original, f.idioma_original,
CASE ss.legendado
     WHEN 'true' THEN 'Legendada'
	 WHEN 'false' THEN 'Dublada'
END AS "Legendada-Dublada", UPPER(ss.idioma_sessao), 
f.genero
FROM sessao ss JOIN filme f ON ( f.cod_filme = ss.cod_filme) ;

-- 44 - Mostrar o filme campeão de arrecadação ( maior renda total)
-- 1o passo -> achar a soma das rendas para cada filme
-- 2o passo -> achar o(s) filme(s) que tem essa renda máxima
-- 3o passo -> trazer outros dados do filme
SELECT campeao.* FROM filme campeao WHERE campeao.cod_filme IN 
   ( SELECT f.cod_filme 
     FROM sessao ss JOIN filme f ON ( f.cod_filme = ss.cod_filme)
     GROUP BY f.cod_filme
     HAVING SUM(ss.renda_sessao) = 
                             ( SELECT MAX(TotalRenda) FROM 
                                ( SELECT f.cod_filme, SUM(ss.renda_sessao) AS TotalRenda
                                   FROM sessao ss JOIN filme f ON ( f.cod_filme = ss.cod_filme)
                                    GROUP BY f.cod_filme) RendaFilme ) );
									
SELECT f.cod_filme, SUM(ss.renda_sessao) AS TotalRenda
    FROM sessao ss JOIN filme f ON ( f.cod_filme = ss.cod_filme)
GROUP BY f.cod_filme
ORDER BY TotalRenda DESC
LIMIT 1 ;

-- 45 -- Funcionario que mais vezes foi escalado como Caixa
SELECT * FROM escala_trabalho ;
SELECT func.* FROM funcionario func WHERE func.cod_funcional IN 
   (SELECT et.cod_funcional 
      FROM escala_trabalho et
      WHERE et.funcao = 'CAIXA' 
      GROUP BY et.cod_funcional
      HAVING COUNT(et.cod_funcional) = 
                    ( SELECT MAX(Qtas_x_caixa) FROM 
                        ( SELECT et.cod_funcional , COUNT(et.cod_funcional) AS Qtas_x_caixa
                          FROM escala_trabalho et
                          WHERE et.funcao = 'CAIXA' 
                          GROUP BY et.cod_funcional) caixa ));
						  
-- 46 - Quem trabalhou nos mesmos dias e horários que o João de Castro e Souza e
-- desempenharam a mesma função - mesma escala com mesma função
SELECT func.nome_func AS "Trabalhou com o João", et.dia_semana, et.cod_horario,
et.funcao
FROM funcionario func JOIN escala_trabalho et
     ON (func.cod_funcional = et.cod_funcional )
WHERE et.dia_semana||'-'||et.cod_horario::char(2)||'-'||et.funcao IN 
      ( SELECT et.dia_semana||'-'||et.cod_horario::char(2)||'-'||et.funcao AS Dia_horario
       FROM funcionario func JOIN escala_trabalho et
       ON (func.cod_funcional = et.cod_funcional )
        WHERE func.nome_func ILIKE 'jo_o%' AND func.nome_func ILIKE '%souza') 
AND func.nome_func NOT ILIKE 'jo_o%' AND func.nome_func NOT ILIKE '%souza';

SELECT CAST(20 AS CHAR(2))
...
INSERT INTO filme (cod_filme, titulo_original, titulo_portugues, idioma_original, sinopse, duracao_min, ano_lancamento, situacao_filme, genero, class_etaria, estudio, nacionalidade)
VALUES 
(5, 'Cidade de Deus', 'Cidade de Deus', 'PORTUGUES', 'Criminalidade nas favelas do Rio.', 130, 2002, 'EM CARTAZ', 'Crime', '18 anos', 'O2 Filmes', 'Brasil'),
(6, 'Interstellar', 'Interestelar', 'INGLES', 'Exploradores viajam pelo espaço.', 169, 2014, 'EM CARTAZ', 'Ficcao Cientifica', '12 anos', 'Paramount', 'EUA'),
(7, 'Amelie', 'O Fabuloso Destino', 'FRANCES', 'Vida de uma jovem em Paris.', 122, 2001, 'EM CARTAZ', 'Comedia Romantica', 'Livre', 'Claudie Ossard', 'França'),
(8, 'Avatar', 'Avatar', 'INGLES', 'Um soldado paraplégico viaja a Pandora.', 162, 2009, 'EM CARTAZ', 'Ficcao Cientifica', '12 anos', '20th Century', 'EUA'),
(9, 'The Dark Knight', 'O Cavaleiro das Trevas', 'INGLES', 'Batman enfrenta o Coringa.', 152, 2008, 'EM CARTAZ', 'Acao', '14 anos', 'Warner Bros', 'EUA'),
(10, 'Spirited Away', 'A Viagem de Chihiro', 'JAPONES', 'Garota em um mundo de espíritos.', 125, 2001, 'EM CARTAZ', 'Animacao', 'Livre', 'Studio Ghibli', 'Japão');

SELECT 
* 
FROM sessao
ORDER BY cod_horario ;

UPDATE horario
SET hora = '17:00:00'
WHERE cod_horario = '17';

UPDATE sessao
SET cod_horario = 16, cod_filme = 10
WHERE num_sessao = 1;
...
INSERT INTO sessao (num_sessao, dt_sessao, publico, renda_sessao, legendado, idioma_sessao, nome_sala, cod_horario, cod_filme, hora_inicio)
VALUES
(19, '2024-06-01', 150, 8000.00, true, 'Ingles', 'Kubrick', 14, 1, '14:00:00'),
(2, '2024-06-01', 120, 9000.00, false, 'Portugues', 'Coppola', 16, 2, '16:00:00'),
(3, '2024-06-01', 180, 12000.00, true, 'Portugues', 'Scorsese', 17,4, '17:00:00'),
(6, '2024-06-01', 200, 13000.00, true, 'Ingles', 'Kubrick', 20, 5, '20:00:00'),
(8, '2024-06-01', 140, 8500.00, false, 'Ingles', 'Coppola', 22, 6, '22:00:00'),
(9, '2024-06-02', 160, 9500.00, true, 'Portugues', 'Scorsese', 14, 7, '14:00:00'),
(10, '2024-06-02', 130, 8000.00, false, 'Portugues', 'Kubrick', 16, 8, '16:00:00'),
(11, '2024-06-02', 170, 11500.00, true, 'Ingles', 'Coppola', 17, 9, '17:00:00'),
(12, '2024-06-02', 110, 7000.00, false, 'Japones', 'Scorsese', 20, 10, '20:00:00'),
(13, '2024-06-02', 190, 12500.00, true, 'Portugues', 'Kubrick', 22, 10, '22:00:00'),
(14, '2024-06-03', 145, 8500.00, true, 'Portugues', 'Coppola', 14, 10, '14:00:00'),
(15, '2024-06-03', 115, 7500.00, false, 'Ingles', 'Scorsese', 16, 1, '16:00:00'),
(16, '2024-06-03', 175, 11000.00, true, 'Portugues', 'Kubrick', 17, 4, '17:00:00'),
(17, '2024-06-03', 135, 9000.00, false, 'Ingles', 'Coppola', 20, 5, '20:00:00'),
(18, '2024-06-03', 155, 10000.00, true, 'Portugues', 'Scorsese', 22, 10, '22:00:00');
...
-- forma 1: subconsulta com NOT IN
...
"-- forma 3: subconsulta com LEFT JOIN e IS NULL não consegui
SELECT f.nome_func
FROM funcionario f
LEFT JOIN escala_trabalho et ON f.cod_funcional = et.cod_funcional
LEFT JOIN (
    SELECT s.cod_horario
    FROM sessao s
    JOIN filme fi ON s.cod_filme = fi.cod_filme
    WHERE fi.nacionalidade = 'Brasil'
) sub ON et.cod_horario = sub.cod_horario
WHERE sub.cod_horario IS NULL;"
...
-- 4. Mostrar as funcionárias (nome) que trabalham juntas na mesma escala de trabalho (dia da semana e horário).Nome Funcionária – Dia Semana – Horário. 
...
-- aqui selecionando funcionari@s do mesmo sexo nos mesmos dias e horarios
SELECT 
    f1.nome_func AS funcionaria1,
    f2.nome_func AS funcionaria2,
    et.dia_semana,
